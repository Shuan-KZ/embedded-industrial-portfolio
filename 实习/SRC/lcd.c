...
{
    //可以看做是一个480行，每行800个元素的矩阵
    unsigned int colors[480][800] = {0};
    ...
    //把颜色数据写入到显示屏文件中
    write(fd_lcd, colors, 480*800*4); //800*480个像素点，每个像素点占4个字节
}
...
void lcd_drwa_num(int num, int x0, int y0, int w, int h, unsigned int color)
{
	...
	while(num != 0)
	{
		lcd_drwa_word(x0-i*(w+5), y0, w, h, nums[num%10], color);
		num = num/10;
